...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, nwd_C: float = 12.8, size_lo: float = 8.0, size_hi: float = 32.0):
        """Initialize the BboxLoss module with regularization maximum and DFL settings."""
        super().__init__()
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None
        self.nwd_C, self.size_lo, self.size_hi = nwd_C, size_lo, size_hi
        self.is_o2m = False  # set by SCPL wiring (E2ELoss) on the one-to-many branch only
        self.tiny_kappa = 0.0  # annealed 0 -> 1 over training; upweights tiny-object o2m supervision
    ...
    def forward(
        ...
        imgsz: torch.Tensor,
        stride: torch.Tensor,
        gamma_px: torch.Tensor | None = None,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Compute IoU (NWD-blended for tiny objects) and DFL losses for bounding boxes."""
        weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)
        if self.is_o2m and gamma_px is not None and self.tiny_kappa:
            weight = weight * (1.0 + self.tiny_kappa * gamma_px[fg_mask].unsqueeze(-1))
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
        if gamma_px is not None:
            g = gamma_px[fg_mask].unsqueeze(-1)
            # stride has shape (na, 1) and broadcasts against (b, na, 4) the same way the DFL-free branch above does
            pb_px, tb_px = (pred_bboxes * stride)[fg_mask], (target_bboxes * stride)[fg_mask]
            nwd = bbox_nwd(pb_px, tb_px, C=self.nwd_C).unsqueeze(-1)
            loss_iou = (((1 - g) * (1.0 - iou) + g * (1.0 - nwd)) * weight).sum() / target_scores_sum
        else:
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        self.assigner = SATaskAlignedAssigner(
            topk=tal_topk,
            num_classes=self.nc,
            alpha=0.5,
            beta=6.0,
        ...
        self.bbox_loss = BboxLoss(m.reg_max, nwd_C=12.8, size_lo=8.0, size_hi=32.0).to(device)
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        gamma_px = tiny_gamma(target_bboxes, self.bbox_loss.size_lo, self.bbox_loss.size_hi)  # pixel-space size gate
...
class E2ELoss:
    ...
    def __init__(self, model: torch.nn.Module, loss_fn=v8DetectionLoss):
        ...
        # final gain
        self.final_o2m = 0.1
        # SCPL: only the one-to-many branch keeps a dense tiny-object weight, annealed 0 -> tiny_kappa_max as
        # the o2m branch's own loss weight decays (i.e. the o2m branch leans harder on tiny objects the less it
        # otherwise contributes), so the assignment stays dense for hard/tiny targets late in training too.
        self.one2many.bbox_loss.is_o2m = True
        self.tiny_kappa_max = 1.0
    ...
    def update(self) -> None:
        ...
        progress = min(self.updates / max(self.one2one.hyp.epochs - 1, 1), 1.0)
        self.one2many.bbox_loss.tiny_kappa = self.tiny_kappa_max * progress
